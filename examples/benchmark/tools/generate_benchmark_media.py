"""Generate cover, poster, and slide video for the JavaScript BarBeR benchmark report.

Requires Pillow and ffmpeg, mirroring the Python/C++ series media pipeline.
...
DISPLAY_NAMES = {
    "dynamsoft-dbr-js": "Dynamsoft Barcode Reader",
...
def benchmark_cover_slide(summary, images, ground_truth):
    image, draw = base_slide()
    title = "ZXing WASM vs. Dynamsoft Barcode Reader"
    draw.text((105, 205), title, font=fit_text(draw, title, 1710, 70, True), fill=WHITE)
    draw.text((108, 302), "BarBeR full dataset benchmark in JavaScript / WASM", font=font(34), fill=MUTED)
    decoders = sorted(summary["decoders"].items(), key=lambda item: item[0] != "dynamsoft-dbr-js")
    colors = [TEAL, BLUE]
...
def main():
    ...
    slides = [
        ...
        title_slide(
            ...
            "The same ImageBitmap and canvas feed both JavaScript/WASM barcode readers",
    ...
    cover.save(args.output / "barcode-benchmark-js-cover.png")
    shutil.copy2(frames / "frame-00.png", args.output / "barcode-benchmark-js-video-poster.png")
    ...
    subprocess.run(
        [
            ...
            "-movflags", "+faststart", str(args.output / "barcode-benchmark-js-video.mp4"),
        ],
        check=True,
    )
